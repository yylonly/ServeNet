...
f_train = open('Data/BERT-ServiceDatasetWithNameMiniBatch-TrainData-old.pickle','rb')
f_test = open('Data/BERT-ServiceDatasetWithNameMiniBatch-TestData-old.pickle', 'rb')
...
def ServeNet():
    ...
    # INPUT - Service Name
    in_name_id = Input(shape=(maxName,), dtype=tf.int32, name="input_word_name_ids_new")
    in_name_mask = Input(shape=(maxName,), dtype=tf.int32, name="input_name_masks_new")
    in_name_segment = Input(shape=(maxName,), dtype=tf.int32, name="segment_name_ids_new")
    ...
    in_id = Input(shape=(maxDes,), dtype=tf.int32, name="input_word_ids_new")
    in_mask = Input(shape=(maxDes,), dtype=tf.int32, name="input_masks_new")
    in_segment = Input(shape=(maxDes,), dtype=tf.int32, name="segment_ids_new")
    ...
    # Merge Features
    all_features = WeightedLayer(name="new_weight_layer")(name_features, description_features)
    ...
    # X = Dropout(0.1)(X)
    output = Dense(50, activation='softmax', kernel_initializer=tf.keras.initializers.TruncatedNormal(stddev=0.02),name="new_dense")(
        all_features)
...
SavePrefix = 'ServeNet-BERT-ServiceName-DataGenerator-More-26x'
checkpointer = [ModelCheckpoint(
    filepath='Data/ServeNet-ServeNet-BERT-ServiceName-DataGenerator-Tran-8-top5.hdf5',
    monitor='val_top_k_categorical_accuracy', verbose=1, save_best_only=True),
                ModelCheckpoint(
                    filepath='Data/ServeNet-ServeNet-BERT-ServiceName-DataGenerator-Tran-8-top1.hdf5',
                    monitor='val_categorical_accuracy', verbose=1, save_best_only=True)]
sgd = tf.keras.optimizers.SGD(learning_rate=0.01,decay=0.005,momentum=0.8, nesterov=True)
model = ServeNet()
model.compile(loss='categorical_crossentropy', optimizer="sgd",
              metrics=[metrics.top_k_categorical_accuracy, metrics.categorical_accuracy])
model.load_weights('Data/ServeNet-ServeNet-BERT-ServiceName-DataGenerator-More-26-top5.hdf5',by_name=True)
for layer in model.layers[3:7]:
    print(layer.name)
    layer.trainable=False
for layer in model.layers[10:17]:
    print(layer.name)
    layer.trainable=False
print(model.get_layer("new_weight_layer").get_weights())

history = model.fit(train_generator(), steps_per_epoch=tranning_steps_per_epoch,
                    validation_data=test_generator(), validation_steps=validation_steps,
                    epochs=140, verbose=1, callbacks=[checkpointer])
